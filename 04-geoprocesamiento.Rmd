...
Cuando tenemos información geográfica y queremos manipularla para generar nuevos datos que nos permitan hacer análisis espaciales, debemos utilizar diferentes herramientas de geoprocesamiento, las cuales son indispensables para la analítica urbana.

Los geoprocesos, que se realizan entre dos o más capas (shapes), pueden ser: unir, recortar, disolver, borrar, fusionar, interseccionar, calcular áreas de influencia (buffer), entre otros. En este capítulo aprenderemos a utilizar las herramientas para geoprocesamiento de datos geográficos incluidas en el paquete `sf`.
...
Entonces, ¿Cómo podemos hacer un mapa coroplético dándole color a las Comunas según la cantidad de propiedades que se alquilan en cada una? Veamos esto en detalle:
Lo primero que debemos hacer es filtrar solo las propiedades que se ubican dentro de CABA y agruparlas por Comuna realizando un conteo de las observaciones que contienen.
...
Luego debemos hacer un `left_join()` entre el shape con los polígonos de las Comunas y los datos que agrupamos:

```{r}
airbnb_con_comunas <- comunas %>%
  left_join(airbnb_con_comunas,
   by="comunas") 
```

Y ahora sí, ya estamos en condiciones de hacer nuestro primer mapa coroplético a partir de la unión de datos espaciales y no espaciales:

```{r}
ggplot() +
  geom_sf(data = airbnb_con_comunas, aes(fill=cantidad), color="gray80") +
...
```{r}
ggplot() +
  geom_sf(data = airbnb_con_comunas, aes(fill=(cantidad/area)*10000), color="gray80") +
    labs(title = "Airbnb por Comuna",
...
Aquí pudimos ver que los resultados son similares a los del mapa anterior y el corredor norte de la Ciudad sigue siendo la zona con mayor cantidad de observaciones, pero si tenemos en cuenta la superficie total de las Comunas, Recoleta pasa a ser el barrio que más densidad de propiedades publicadas en Airbnb tiene.
